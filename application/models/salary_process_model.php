...
class Salary_process_model extends CI_Model{
	...
	function pay_sheet($year, $month,$process_check,$grid_emp_id)
	{
		...
		//DATE CREATION FOR FRACTIONAL DATE SALARY SYSTEM
		$prvious_month 	= date('m',strtotime($next_month_year));
		$prvious_year 	= date('Y',strtotime($next_month_year));

		$last_date = date("t", mktime(0, 0, 0, $month_v, 1, $year_v));

		$FS_on_date 	= date("Y-m-d", mktime(0, 0, 0, $month_v, 1, $year_v));
		$FS_off_date 	= date("Y-m-d", mktime(0, 0, 0, $month_v, $last_date, $year_v));

		$num_of_days = $this->get_days($FS_on_date, $FS_off_date);
        $day_of_week = $this->get_how_many_weeked_between_dates($FS_on_date,$FS_off_date);
		...
		//print_r($result);
	  	$arr = array(12010341,12010414);
		
		$this->db->select("emp_id,gross_sal,emp_sal_gra_id,emp_desi_id,emp_join_date,salary_type,emp_sec_id,ot_entitle,com_gross_sal,emp_dept_id,emp_line_id,emp_cat_id,floor_id,salary_draw");
		$this->db->where("unit_id",$unit_id);
		$this->db->where_in("emp_id",$grid_emp_id);
		// $this->db->where("emp_id","19040006");
		// $this->db->where_in("emp_id",$arr);
		// $this->db->where("emp_sec_id","18");
		...
		else
		{
			...
			foreach($query->result() as $rows)
			{
				...
				$salary_process_eligibility = $this->salary_process_eligibility($emp_id, $FS_on_date, $FS_off_date);
				...
				$data_com["stop_salary"]	= $stop_salary;
				...
				$salary_month = trim(substr($FS_off_date,0,7));

				$join_month = trim(substr($doj,0,7));

				$resign_check 	= $this->resign_check($emp_id, $FS_on_date, $FS_off_date);
				$left_check 	= $this->left_check($emp_id, $FS_on_date, $FS_off_date);
				if($resign_check != false and $salary_month == $join_month)
				{
					
					$left_or_resign_date = $resign_check['resign_date'];
					$resign_after_absent = $this->get_days($left_or_resign_date,$FS_off_date);
					$doj_before_absent = $this->get_days($FS_on_date, $doj);					
				...
				// echo $emp_id.'='.$absent.'='.$FS_on_date.'='.$FS_off_date;
				$absent = "A";
				$absent = $this->attendance_check($emp_id,$absent,$FS_on_date,$FS_off_date);
				$attend = "P";
				$attend = $this->attendance_check($emp_id,$attend,$FS_on_date,$FS_off_date);

				$w_h_ot = $this->w_h_ot_hour_sum($emp_id,$FS_on_date,$FS_off_date);
				if(empty($w_h_ot)){
					$w_h_ot = 0;
				}

				$modify_hour = $this->modify_hour_sum($emp_id,$FS_on_date,$FS_off_date);
				
				$leave_type = "cl";
				$cas_leave = $this->leave_db($emp_id, $FS_on_date,$FS_off_date, $leave_type);
				$leave_type = "sl";
				$sick_leave = $this->leave_db($emp_id, $FS_on_date,$FS_off_date, $leave_type);
				$leave_type = "el";
				$earn_leave = $this->leave_db($emp_id, $FS_on_date,$FS_off_date, $leave_type);
				$leave_type = "ml";
				$m_leave = $this->leave_db($emp_id, $FS_on_date,$FS_off_date, $leave_type);
				$leave_type = "wp";
				$wp_leave = $this->leave_db($emp_id, $FS_on_date,$FS_off_date, $leave_type);
				$leave_type = "do";
				$do_leave = $this->leave_db($emp_id, $FS_on_date,$FS_off_date, $leave_type);
				...
				$weekend = "W";
				$weekend = $this->attendance_check($emp_id,$weekend,$FS_on_date,$FS_off_date);
				$holiday = "H";
				$holiday = $this->attendance_check($emp_id,$holiday,$FS_on_date,$FS_off_date);
				...
				//LATE DEDUCTION
				//$late_count = $this->get_late_count($emp_id,$year,$month);$FS_off_date
				$late_count = $this->get_late_count($emp_id,$FS_on_date,$FS_off_date);
				...
				if($ot_check == 0)
				{
					$ot_hour = $this->ot_hour($emp_id, $FS_on_date, $FS_off_date);
					if($ot_hour == '')
					{
						$ot_hour = 0;
					}
				...
				if($ot_check == 0)
				{
					$collect_eot_hour = $this->eot_hour($emp_id, $FS_on_date, $FS_off_date);
					//THIS EOT IS INTRODUCE FOR SUPER ADMIN PRIVILEGE IN ACL
					$eot_hour_for_sa = $this->eot_hour_for_super_admin($emp_id, $FS_on_date, $FS_off_date);
					
					// $modify_eot_hour = $this->modify_eot_hour($emp_id, $FS_on_date, $FS_off_date);
					if($ot_hour == '')
					{
						$collect_eot_hour = 0;
						// $modify_eot_hour = 0;
				...
				//echo $net_pay;
				//COMPLIENCE
				// $net_pay_com = $gross_sal_com + $att_bouns + $ot_amount - $total_deduction ;
	...
	/*function modify_eot_hour($emp_id, $FS_on_date, $FS_off_date)
	{
